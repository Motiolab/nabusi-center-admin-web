...
import { useMutationUpdateWellnessTicket, useQueryGetWellnessTicketDetailById } from "@/entities/wellnessticket/model";
...
const UpdateWellnessTicket = ({ id }: IProps) => {
    ...
    const updateMutation = useMutationUpdateWellnessTicket(() => {
        queryClient.invalidateQueries({ queryKey: ['getWellnessTicketDetailById', selectedCenterId, id] })
        navigate(-1);
    });
    const { data: wellnessTicketDetail, isError, error } = useQueryGetWellnessTicketDetailById(selectedCenterId, id);

    useEffect(() => {
        if (!wellnessTicketDetail) return;
        setName(wellnessTicketDetail.name);
        setType(wellnessTicketDetail.type);
        setPrice(wellnessTicketDetail.price);
        setDiscountValue(wellnessTicketDetail.discountValue);

        if (wellnessTicketDetail.usableDate % 30 === 0) {
            setUsableMonth(wellnessTicketDetail.usableDate / 30);
        } else {
            setIsUsableDateManually(false)
            setUsableDate(wellnessTicketDetail.usableDate)
        }
        setTotalUsableCnt(wellnessTicketDetail.totalUsableCnt)
        setLimitType(wellnessTicketDetail.limitType)
        setLimitCnt(wellnessTicketDetail.limitCnt)
        setBackgroundColor(wellnessTicketDetail.backgroundColor)
        setWellnessClassIdList(wellnessTicketDetail.wellnessClassNameList.map((e: IGetWellnessClassIdAndName) => e.id))
    }, [wellnessTicketDetail])
    ...
    const clickCreateButton = () => {
        ...
        const request: IUpdateWellnessTicketAdminRequestV1 = {
            id,
    ...
    return <>
        ...
        <Flex align="center" style={{ marginTop: 24, justifyContent: 'space-between' }}>
            <Button onClick={() => navigate(`/wellness-ticket/${id}`)}>취소</Button>
            <Button type="primary" onClick={clickCreateButton}>수정하기</Button>
        </Flex>
    </>
}

export default UpdateWellnessTicket