...
import { editNotice, getNotice } from "@/entities/notice/api";
...
const NoticeEdit = () => {
    ...
    const onClickEditButton = () => {
        if (!id || !title || !htmlContents || isPinTop === undefined || isShow === undefined || loading) return;
        const value = { id, title, htmlContents, isPinTop, isShow }
        setLoading(true)
        editNotice(value, selectedCenterId)
    ...
    return <>
            ...
            <div style={{ backgroundColor: "var(--Base-Base-White)", height: "calc(100% - 45px)", color: "var(--Base-Base-Black)" }}>
                <div style={{ display: "flex", flexDirection: "column", gap: "var(--Spacingl)", marginBottom: "90px", padding: "var(--Spacingl)" }}>
                    ...
                    <div style={{ display: "flex", gap: "var(--Spacingsm)" }}>
                        ...
                        {/* <CustomEditor htmlContents={htmlContents} setHtmlContents={setHtmlContents} /> */}
                    </div>
                    <div style={{ display: "flex", gap: "var(--Spacingsm)", height: "28px", alignItems: "center" }}>
                        <div className="body-content-bold" style={{ minWidth: "100px" }}>
                            게시 여부
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "var(--Spacingl)", borderTop: "1px solid var(--Neutrals-Neutrals200)" }}>
                    ...
                    <Button style={{ color: "var(--Primary-Primary)", borderColor: "var(--Primary-Primary)" }} onClick={() => setEditModalStatus(true)}>저장하기</Button>
                </div>
            </div>
        </div>
        <Modal centered styles={{ content: { padding: "var(--Spacingml) var(--Spacingbase) var(--Spacingml) var(--Spacingbase)" } }} open={editModalStatus} footer={false} closeIcon={false} width={436} onCancel={() => setEditModalStatus(false)}>
            ...
            <div className="body-content-standard" style={{ color: "var(--Neutrals-Neutrals700)", marginBlock: "20px" }}>
                작성하신 내용을 저장하시겠습니까?<br />
            </div>
            <div style={{ display: "flex", gap: "var(--Spacingbase)" }}>
                <Button className="body-content-accent" style={{ height: "52px", width: "50%" }} onClick={() => setEditModalStatus(false)}>돌아가기</Button>
                <Button disabled={loading} className="body-content-accent" style={{ height: "52px", width: "50%" }} onClick={onClickEditButton} type="primary">{loading ? "저장중" : "저장하기"}</Button>
            </div>
        </Modal>
        <Modal centered styles={{ content: { padding: "var(--Spacingml) var(--Spacingbase) var(--Spacingml) var(--Spacingbase)" } }} open={cancelModalStatus} footer={false} closeIcon={false} width={436} onCancel={() => setCancelModalStatus(false)}>
            ...
            <div className="body-content-standard" style={{ color: "var(--Neutrals-Neutrals700)", marginBlock: "20px" }}>
                수정을 그만하시겠습니까?<br />
                수정한 내용은 저장되지 않습니다.<br />
            </div>
            <div style={{ display: "flex", gap: "var(--Spacingbase)" }}>
                <Button className="body-content-accent" style={{ height: "52px", width: "50%" }} onClick={() => setCancelModalStatus(false)}>돌아가기</Button>
                <Link to={"/home/customercenter/noticelist"}><Button className="body-content-accent" style={{ height: "52px", width: "50%", color: "var(--Error-Error)", border: "1px solid var(--Error-Error)" }}>그만하기</Button></Link>
            </div>
        </Modal>
    </>
}
