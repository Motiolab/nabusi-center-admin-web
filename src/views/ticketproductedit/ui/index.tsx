...
import { editTicketProduct, getTicketProductById } from "@/entities";
import { useSelector } from "react-redux";
import { RootState } from "@/store";
import { useNavigate, useSearchParams } from "react-router-dom";

const TicketProductEditView = () => {
    const [form] = useForm();
    ...
    const [limitTypePeriod, setLimitTypePeriod] = useState<string>("WEEKLY");
    ...
    useEffect(() => {
        if (!id) return;
        getTicketProductById(id, selectedCenterId)
            .then(res => {
                setTicketProductData(res.data.body)
                setTypeStatus(res.data.body.type)
                setLastPrice(res.data.body.price * (res.data.body.discountPercent / 100))
                setLimitTypePeriod(res.data.body.limitTypePeriod)
                setLimitCntPeriod(res.data.body.limitCntPeriod)
            })
            .catch()
    }, [])
    const onFinish = (values: any) => {
        const requestBody = { ...values, id: id, classTypeIdList: values.classTypeIdList?.filter((q: number) => q !== 0) }
        setLoading(true);
        editTicketProduct(requestBody, selectedCenterId)
    ...
    return <>
        <div style={{ height: "100%" }}>
            ...
            <div style={{ backgroundColor: "var(--Base-Base-White)", height: "calc(100% - 40px)", display: "flex", flexDirection: "column", gap: "var(--Spacingl)" }}>
                {ticketProductData
                    && <Form form={form} onFinish={onFinish} colon={false} labelAlign="left" labelCol={{ style: { width: "136px", alignItems: "center", display: "flex" } }} initialValues={ticketProductData} >
                            ...
                            <div style={{ marginInline: "var(--Spacingl)", marginBottom: "var(--Spacingl)" }} className="body-content-bold" >
                                ...
                                <div style={{ display: "flex", height: "44px", marginBottom: "24px", gap: "var(--Spacingsm)" }}>
                                    <Form.Item label="수강권 종류" name="type" initialValue={"COUNT"} className={styles.ailineItemCenter}>
                                        <Radio.Group disabled className="body-content-standard" style={{ width: "300px", height: "44px", display: "flex", alignItems: "center" }} >
                                ...
                                <div style={{ display: "flex", height: "44px", marginBottom: "24px", gap: "var(--Spacingsm)" }}>
                                    ...
                                    <div style={{ display: "flex", alignItems: "center" }} className="body-content-standard">원<span className="body-caption-standard" style={{ color: "var(--Neutrals-Neutrals500)", marginLeft: "12px" }}>※ 변경 시 이후 발급되는 수강권부터 금액이 적용됩니다.</span></div>
                                </div>
                                <div style={{ display: "flex", height: "44px", marginBottom: "24px", gap: "var(--Spacingsm)" }}>
                                    ...
                                    <div style={{ display: "flex", alignItems: "center" }} className="body-content-standard">%<span className="body-caption-standard" style={{ color: "var(--Neutrals-Neutrals500)", marginLeft: "12px" }}>※ 변경 시 이후 발급되는 수강권부터 할인율이 적용됩니다.</span></div>
                                </div>
                                ...
                                <div style={{ display: "flex", height: usebleDateHandwritten ? "255px" : "185px", marginBottom: "24px", gap: "var(--Spacingsm)" }}>
                                    <Form.Item label="유효기간" name="usableDate" labelCol={{ style: { display: "flex", alignItems: "flex-start", width: "136px" } }} initialValue={usableDate / 30}>
                                        ...
                                        <div style={{ display: "flex", flexDirection: "column", gap: "var(--Spacingl)" }}>
                                            ...
                                            <div>
                                                <span className="body-caption-standard" style={{ color: "var(--Neutrals-Neutrals500)", marginLeft: "12px" }}>※ 변경 시 이후 발급되는 수강권부터 기간이 적용됩니다.</span>
                                            </div>
                                        </div>
                                    </Form.Item>
                                </div>
                                ...
                                {typeStatus === "COUNT" && <div style={{ display: "flex", height: "44px", marginBottom: "24px", gap: "var(--Spacingsm)" }}>
                                    ...
                                    <div style={{ display: "flex", alignItems: "center" }} className="body-content-standard">회<span className="body-caption-standard" style={{ color: "var(--Neutrals-Neutrals500)", marginLeft: "12px" }}>※ 변경 시 이후 발급되는 수강권부터 횟수가 적용됩니다.</span></div>
                                </div>}
                                <div style={{ display: "flex", height: "44px", marginBottom: "24px", gap: "var(--Spacingsm)" }}>
                                    <Form.Item label="이용제한" name="limitTypePeriod" style={{ height: "44px" }} initialValue={"WEEKLY"}>
                                        <Radio.Group className="body-content-standard" style={{ height: "44px", display: "flex", alignItems: "center", width: "250px" }} onChange={(e) => setLimitTypePeriod(e.target.value)}>
                                ...
                                <div style={{ display: "flex", height: "44px", marginBottom: "24px", gap: "var(--Spacingsm)" }}>
                                    <Form.Item name="classTypeIdList" label="예약 가능 수업 종류" style={{ height: "44px" }} >
                                        <Select mode="multiple" className={`body-content-standard ${styles.overFlowScroll}`} style={{ width: "300px", height: "44px" }}>
                                            <Select.Option value={0}>수업 종류 전체</Select.Option>
                                            <Select.Option value={1}>TTC</Select.Option>
                                            <Select.Option value={2}>그룹</Select.Option>
                                            <Select.Option value={3}>종류 4</Select.Option>
                                            <Select.Option value={4}>종류 5</Select.Option>
                                            <Select.Option value={5}>종류 6</Select.Option>
                                        </Select>
                                    </Form.Item>
                                    <div style={{ height: "44px", display: "flex", alignItems: "center" }} className="body-content-standard">수업만 예약 가능<span className="body-caption-standard" style={{ color: "var(--Neutrals-Neutrals500)", marginLeft: "12px" }}>※ 변경 시 기존 발급된 수강권에도 일괄 적용됩니다.</span></div>
                                </div>
                            </div>
                        </div>
                        <div style={{ display: "flex", gap: "var(--Spacingml)", justifyContent: "right", padding: "var(--Spacingl)" }}>
                            ...
                            <Button onClick={() => setCheckSaveModalStatus(true)} type="primary">{loading ? "저장 중" : "저장하기"}</Button>
                        </div>
                    </Form >}
            </div >
        ...
        <Modal centered styles={{ content: { padding: "var(--Spacingml) var(--Spacingbase) var(--Spacingml) var(--Spacingbase)" } }} open={checkSaveModalStatus} footer={false} closeIcon={false} width={436} onCancel={() => setCheckSaveModalStatus(false)}>
            ...
            <div style={{ display: "flex", gap: "var(--Spacingbase)" }}>
                <Button className="body-content-accent" style={{ height: "52px", width: "50%" }} onClick={() => setCheckSaveModalStatus(false)}>돌아가기</Button>
                <Button className="body-content-accent" style={{ height: "52px", width: "50%" }} onClick={() => onFinish(form.getFieldsValue())} type="primary">저장하기</Button>
            </div>
        </Modal>
        <Modal centered styles={{ content: { padding: "var(--Spacingml) var(--Spacingbase) var(--Spacingml) var(--Spacingbase)" } }} open={checkCancelModalStatus} footer={false} closeIcon={false} width={436} onCancel={() => setCheckCancelModalStatus(false)}>
            ...
            <div style={{ display: "flex", gap: "var(--Spacingbase)" }}>
                <Button className="body-content-accent" style={{ height: "52px", width: "50%" }} onClick={() => setCheckCancelModalStatus(false)}>돌아가기</Button>
                <Button className="body-content-accent" style={{ height: "52px", width: "50%" }} onClick={() => navigate(-1)} type="primary" danger>그만하기</Button>
            </div>
        </Modal>
    </>
}
